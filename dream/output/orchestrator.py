...
def get_expect_all():
    ...
    for dog_index in tobe_trade_list:
        ...
        if re.search(r'\d{6}', dog_code_filter):   # Search if have number like '250117'
            log.get(log_name).info('Detect option[%s]'%(dog_code_filter))
        else:
...
def long_term_trade(house_dict, dog_opt, dog_id):
    ...
    try:
        ...
        log.get(log_name).info('[long_term_trade]register realtime for: %s %s'%(dog_id, str(recv_dict)))
    except Exception as e:
        log.get(log_name).error('Exception captured in long_term_trade register_dog: %s'%(str(e)))
    try:
        ...
        if last_price < 0.0 or trough_prob < 0.0 or peak_prob < 0.0:
            ...
            log.get(log_name).error(content.replace('\n', ''))
            return content
        ...
        #log.get(log_name).info('[%s]trough_: %.2f%% peak:%.2f%% last_price:%.2f [%.2f, %.2f]'%(
        ...
        if buy_price > 0 and available_cash > 0:
            ...
            if (quantity * buy_price) > available_cash:
                log.get(log_name).info('[%s] No enough money to buy[%d], all in ......'%(dog_id, quantity))
                all_in_quantity = available_cash // buy_price
                if all_in_quantity * 3 < quantity:
                    log.get(log_name).info('[%s] no need to buy, too less[%d], ignore ......'%(dog_id, all_in_quantity))
                    quantity = 0
                else:
            ...
            quantity = get_last_inject(curr_quantity, float(get_global_config('next_inject_factor')))
            if quantity <= 0:
                ...
                log.get(log_name).info('[%s] Curr[%d], Cost[%.2f, %.2f] to be sell[%d]'%(dog_id, curr_quantity, cost_price, fee, quantity))
        ...
        while (retry_time > 0):
            try:
                ...
                retry_time -= 1
                log.get(log_name).error('Exception captured in trade submit_order: %s, retry:%d'%(str(e), retry_time))
                time.sleep(10)
        if retry_time == 0:
            content = 'Exception captured in trade submit_order: %s\n'%(str(e))
        return content
    except Exception as e:
        content = 'Exception captured in trade: %s\n'%(str(e))
        log.get(log_name).error(content.replace('\n',''))
        return content
    ...
    while(True):
        ...
        if retry_time == 0:
            ...
            try:
                ...
                log.get(log_name).error('Exception captured in order_monitor bark[%s]: %s'%(content, str(e)))
...
def order_monitor(order_id, dog_id, price, quantity, side):
    ...
    try:
        recv_dict = register_dog(dog_id)
        log.get(log_name).info('[order_monitor]register realtime for: %s %s'%(dog_id, str(recv_dict)))
    except Exception as e:
...
def short_term_trade(house_dict):
    ...
    try:
        for index in trade_list:
            ...
            log.get(log_name).info('[short_term_trade]register realtime for: %s %s'%(index, str(recv_dict)))
    except Exception as e:
    ...
    while(True):
        ...
        for target in trade_list:
            ...
            if math.isnan(trough_prob) or math.isnan(peak_prob):
                log.get(log_name).error('%s: Probability Error math.nan [%.2f%% , %.2f%%]'%(target, trough_prob, peak_prob))
                error_cnt += 1
                continue
            ...
            if action_type != '':
                ...
                if last > 0.01 and price_diff < price_float_th:
                    log.get(log_name).info('[%s]%s, too less diff last[%.2f], Now[%.2f] RequireDiff[%.2f%%]'%(target, 
                ...
                while (retry_time > 0):
                    try:
                        recv_dict = submit_order(target, action_type, now_price, quantity)
                        content += '[%s] %s [%d] in %.2f\n'%(target, action_type, quantity, now_price)
                        log.get(log_name).info('[%s] %s [%d] in %.2f'%(target, action_type, quantity, now_price) + ', ret: %s'%(str(recv_dict)))
                        if action_type == 'buy':
                        ...
                        retry_time -= 1
                        log.get(log_name).error('Exception in short_term_trade submit_order: %s, retry:%d'%(str(e), retry_time))
                        time.sleep(10)
                if retry_time == 0:
