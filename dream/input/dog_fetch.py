...
def get_dog_daily_hist(dog_id, **kwargs):
    try:
...
def get_capital_flow(dog_id):
    df = ak.stock_individual_fund_flow(stock="159509", market=get_market("159509"))
...
def main():
    ...
    for dog_index in dog_list:
        if (not db.is_table_exist(dog_index)):      # New a table to insert
            ...
            df1 = get_dog_daily_hist(dog_index)
            df2 = get_capital_flow(dog_index)
            df = pd.merge(df1, df2, on='Date', how='left')
            df.fillna(0, inplace=True)
        else:
            current_date = datetime.datetime.now().strftime('%Y%m%d')
            start_date = (datetime.datetime.now() - datetime.timedelta(days=15)).strftime('%Y%m%d')    # 15 days ago
            df1 = get_dog_daily_hist(dog_index, start_date=start_date, end_date=current_date)
            df2 = get_capital_flow(dog_index)
            df = pd.merge(df1, df2, on='Date', how='left')
            df.fillna(0, inplace=True)
